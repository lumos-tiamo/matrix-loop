...
# CORS 与鉴权中间件在下方「安全加固」区块统一注册
# （需先 load_env_file() 才能读到 CORS_ORIGINS / APP_TOKEN）。
...
# ══════════════════════════════════════════════════════════════════════
# 安全加固 + 无头「内容作业」API 接入（新增，见 docs/JOB-API.md）
# 全部环境变量驱动、带安全默认；同机部署下 loop 走 127.0.0.1 回环免 token。
# ══════════════════════════════════════════════════════════════════════
def _harden_flag(name, default):
    raw = os.getenv(name)
    if raw is None:
        return default
...
try:
    SERVER_PORT = int(os.getenv("PORT", "3000") or "3000")
except ValueError:
    SERVER_PORT = 3000
HEADLESS = _harden_flag("HEADLESS", False)
ENABLE_SELF_UPDATE = _harden_flag("ENABLE_SELF_UPDATE", True)
...
if _cors_env == "*":
    ...
    CORS_ALLOW_ORIGINS = [f"http://127.0.0.1:{SERVER_PORT}", f"http://localhost:{SERVER_PORT}"]


# 鉴权中间件：本机回环放行；远程需 X-App-Token == APP_TOKEN；未配置 token 则拒绝远程。
@app.middleware("http")
async def _app_auth_guard(request: Request, call_next):
    client_host = request.client.host if request.client else ""
    if client_host in _LOOPBACK_HOSTS:
        return await call_next(request)
    if not APP_TOKEN:
        return JSONResponse({"detail": "远程访问被拒绝：服务未配置 APP_TOKEN"}, status_code=403)
...
# CORS 最后注册 → 处于最外层，优先处理浏览器预检 OPTIONS（不经过鉴权）。
app.add_middleware(
    CORSMiddleware,
...
except Exception as _jobs_exc:  # 作业层出问题不应拖垮主服务
    job_service = None
    logging.getLogger("jobs").exception("作业层初始化失败: %s", _jobs_exc)
...
async def request_validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    # jsonable_encoder：pydantic 校验器抛出的 ValueError 会被塞进 error ctx，
    # 直接 json.dumps 会崩（非可序列化对象）。用 encoder 兜底，全站更健壮。
    return JSONResponse(
        status_code=422,
        content={
...
def schedule_self_restart(delay_seconds: int = 3) -> bool:
    ...
    if not ENABLE_SELF_UPDATE:
        logging.getLogger("update").warning("self-restart 被禁用（ENABLE_SELF_UPDATE=0）")
        return False
...
def update_from_github(req: UpdateRequest = UpdateRequest()):
    if not ENABLE_SELF_UPDATE:
        raise HTTPException(status_code=403, detail="自更新已被禁用（设置 ENABLE_SELF_UPDATE=1 可开启）")
...
async def get_global_token(request: Request):
    ...
    saved_token = modelscope_api_key() or ""
    if not saved_token and os.path.exists(GLOBAL_CONFIG_FILE):
        try:
            with open(GLOBAL_CONFIG_FILE, 'r', encoding='utf-8') as f:
                saved_token = json.load(f).get("modelscope_token", "") or ""
        except:
            saved_token = ""
    # 仅本机回环返回明文（供 UI 预填）；其余一律打码，避免密钥出网。
    client_host = request.client.host if request.client else ""
    if saved_token and client_host not in _LOOPBACK_HOSTS:
        masked = (saved_token[:4] + "****" + saved_token[-4:]) if len(saved_token) > 8 else "****"
        return {"token": "", "masked": masked, "has_token": True}
